...
class profile::test_exporter {
  ...
  # and create them on the host this exporter class is applied too. 
  Host <<| tag == 'bens_hosts' |>>
  ...
  $query_results.each | Hash $results | {
    # We want to check if the resource is exported before printing. Otherwise will also get the resources which have been realised 
